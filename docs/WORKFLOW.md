...
When `needsPlan=true`, needsPlan=true requires spec before plan:
- `leader` runs `dtt-brainstorming` first
- spec and plan outputs follow current conversation language
