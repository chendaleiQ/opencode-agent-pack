...
def _load_json_list(raw: str) -> List[str]:
    try:
        ...
        raise SystemExit("allowed providers must be a JSON array of non-empty strings") from exc
...
def detect_provider_candidates(config_dir: Path, data_dir: Path, cache_dir: Path) -> List[str]:
    ...
    if isinstance(active_provider, str) and active_provider and active_provider not in seen:
        seen.add(active_provider)
        candidates.append(active_provider)

    candidates.sort()
    if isinstance(active_provider, str) and active_provider and active_provider in candidates:
...
def read_allowed_providers(settings_obj: Dict[str, Any]) -> List[str]:
    ...
    if not isinstance(pack_obj, dict):
        raise SystemExit("invalid settings.json: expected opencodeAgentPack to be a JSON object")
    if "allowedProviders" not in pack_obj:
        return []
    raw = pack_obj.get("allowedProviders")
    if not isinstance(raw, list):
        raise SystemExit("invalid settings.json: expected allowedProviders to be a JSON array")
    if any(not isinstance(item, str) or not item for item in raw):
        raise SystemExit("invalid settings.json: expected allowedProviders entries to be non-empty strings")
    return list(raw)
...
def _write_settings(path: Path, settings_obj: Dict[str, Any]) -> None:
    ...
    path.write_text(json.dumps(settings_obj, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    settings_path = Path(args.settings_path) if args.settings_path else config_dir / "settings.json"
